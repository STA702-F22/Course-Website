...
For this lab, you will need one stan file. Download here:
  ...
  + https://sta-602l-s21.github.io/Course-Website/labs/lab-07-hmc_norm_example.stan.
...
for two univariate parameters $\theta_1$ and $\theta_2$. As we have already seen in class, MCMC methods like the Gibbs sampling, will move iteratively through the plane, transitioning from point to point and producing a set of $S$ samples
...
Consider data generated from a bivariate normal distribution with mean parameter $\boldsymbol{\theta} = (\theta_1, \theta_2)$ and known covariance matrix $\Sigma$ and suppose we place independent normal priors on $\theta_1, \theta_2$:

$$
\begin{aligned}
\boldsymbol{X}_1, \dots, \boldsymbol{X}_n &\sim N_2 \left(\boldsymbol{\theta}, \Sigma \right); \\
\theta_j &\sim N(0, 1)~~~~~~j=1,2.
\end{aligned}
$$

where each $\boldsymbol{X}_i = (X_{i1},X_{i2})^T$. Suppose that the covariance matrix $\Sigma$ is known and has the form
...
with known correlation parameter $\rho$.

So far with the multivariate normal, we have derived the multivariate posterior density $p(\boldsymbol{\theta} | \Sigma, \boldsymbol{X})$. However, for the purposes of this exercise, we will sample from the following univariate full conditional densities instead.

$$
p(\theta_1 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_2) \\
p(\theta_2 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_1).
$$
...
1. What is the conditional density $p(\theta_1 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_2)$?
2. What is the conditional density $p(\theta_2 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_1)$?
...
<!-- $$ -->
<!-- \begin{aligned} -->
<!-- \theta_1 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_2 &\sim N\left(\frac{\sum_{i} X_{i1} + \rho(n\theta_2 - \sum_{i} X_{i2})}{n+1-\rho^2}, ~(1-\rho^2)/(n+1-\rho^2) \right) \\ -->
<!-- \theta_2 | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho, \theta_1 &\sim N\left(\frac{\sum_{i} X_{i2} + \rho(n\theta_1 - \sum_{i} X_{i1})}{n+1-\rho^2}, ~(1-\rho^2)/(n+1-\rho^2)\right) -->
<!-- \end{aligned} -->
<!-- $$ -->
...
3. Specifically, write a function called `normal_gibbs_sampler` that takes as arguments: **(1)** the number of samples desired (`S`), **(2)** an $n \times 2$ matrix of data values (`X`), and **(3)** the given correlation parameter (`rho`).
    ...
    Have this function return an $S \times 2$ matrix of samples containing $[\theta^{(1)}_1, \theta^{(1)}_2], \dots, [\theta^{(S)}_1, \theta^{(S)}_2]$, your realizations from the joint posterior $p(\boldsymbol{\theta} | \boldsymbol{X}_1, \dots, \boldsymbol{X}_n, \rho)$.
